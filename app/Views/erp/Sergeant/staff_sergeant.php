...
if($user_info['user_type'] == 'staff'){
   ...
   $main_designation = $DesignationModel->where('company_id', $user_info['company_id'])->findAll();
} else {
	$main_department = $DepartmentModel->where('company_id', $usession['sup_user_id'])->findAll();
	$main_designation = $DesignationModel->where('company_id', $usession['sup_user_id'])->findAll();
}
$get_animate ='';
?>
<?php if(in_array('news1',staff_role_resource()) || in_array('department1',staff_role_resource()) || in_array('designation1',staff_role_resource()) || in_array('sergeant1',staff_role_resource()) || in_array('policy1',staff_role_resource()) || $user_info['user_type'] == 'company') { ?>
...
<div id="smartwizard-2" class="border-bottom smartwizard-example sw-main sw-theme-default mt-2">
  <ul class="nav nav-tabs step-anchor">
    ...
    <li class="nav-item active"> <a href="<?= site_url('erp/sergeant-list');?>" class="mb-3 nav-link"> <span class="sw-done-icon feather icon-check-circle"></span> <span class="sw-icon feather icon-list"></span>
...
<div class="row m-b-1 <?php echo $get_animate;?>">
  ...
  <div class="col-md-4">
    <div class="card">
      ...
      <div class="card-body">
        <?php $attributes = array('name' => 'add_sergeant', 'id' => 'xin-form', 'autocomplete' => 'off');?>
        <?php $hidden = array('user_id' => 1);?>
        <?php echo form_open('erp/sergeant/add_sergeant', $attributes, $hidden);?>
        ...
        <div class="form-group" id="designation_ajax">
          <label for="name">
            <?= lang('Dashboard.left_designation');?> <span class="text-danger">*</span>
          </label>
        ...
        <div class="form-group">
          <label for="name">
            <?= lang('Dashboard.left_sergeant_name');?> <span class="text-danger">*</span>
          </label>
          <input type="text" class="form-control" name="sergeant_name" placeholder="<?= lang('Dashboard.left_sergeant_name');?>">
        </div>
        <div class="form-group">

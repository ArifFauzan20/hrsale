...
$main_department = $DepartmentModel
  ->where('company_id', $company_id)
  ->where('department_head!=', 0)->findAll();
$get_animate = '';
?>
  ...
  $(function() {
              ...
              <?php foreach ($idesignation as $sdesign) { ?>
                ...
                <?php if ($iuser_count > 0) { ?>
                  <?php $iuser = $UsersModel->where('user_id', $sdesign['user_id'])->first(); ?> {
                    ...
                    <?php $subserge = $SergeantModel->where('department_id', $idepartment['department_id'])->where('designation_id', $sdesign['designation_id'])->first(); ?>
                    <?php $isergeant = $StaffdetailsModel->where('sergeant_id', $subserge['sergeant_id'])->findAll(); ?> 'children': [
                      <?php foreach ($isergeant as $ssergeant) { ?>
                        <?php $esergeant = $SergeantModel->where('sergeant_id', $ssergeant['sergeant_id'])->first(); ?>
                        <?php $iuser_count = $UsersModel->where('user_id', $ssergeant['user_id'])->countAllResults(); ?>
                        <?php if ($iuser_count > 0) { ?>
                          <?php $iuser = $UsersModel->where('user_id', $ssergeant['user_id'])->first(); ?> {
                            'profile': '<?= base_url(); ?>/public/uploads/users/thumb/<?= $iuser['profile_photo']; ?>',
                            'name': '<?= $iuser['first_name'] . ' ' . $iuser['last_name']; ?>',
                            'title': '<?= $esergeant['sergeant_name']; ?>',
                            // <?php $subserge = $SergeantModel->where('department_id', $idepartment['department_id'])->where('designation_id', $sdesign['designation_id'])->first(); ?>
                            // <?php $isergeant = $StaffdetailsModel->where('sergeant_id', $subserge['sergeant_id'])->findAll(); ?> 'children': [
                            //   <?php foreach ($isergeant as $ssergeant) { ?>
                            //     <?php $esergeant = $SergeantModel->where('sergeant_id', $ssergeant['sergeant_id'])->first(); ?>
    ...
    var oc = $('#chart-container').orgchart({
      ...
      'createNode': function($node, data) {
        ...
        if (data.title === "<?= $user_chart['company_name']; ?>") {
          var assistantNode =
            `<div class="assistant-node">
                <div class="connector"></div>
                <div class="title">Luciana</div>
                <div class="content-chart">Asistant</div>
                <img class="avatar" src="http://localhost:8081/hrsale/public/uploads/users/thumb/a-sm.jpg" alt="" />
              </div>`;
          $node.append(assistantNode);
        }
        if (data.title === "Liutenant") {
          var assistantNode =
            `<div class="assistant-node">
              <div class="connector"></div>
              <div class="title">Nurbuat</div>
              <div class="content-chart">Asistant</div>
              <img class="avatar" src="http://localhost:8081/hrsale/public/uploads/users/thumb/a-sm.jpg" alt="" />
            </div>`;
          $node.append(assistantNode);
